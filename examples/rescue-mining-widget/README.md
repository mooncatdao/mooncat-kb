...
This example does not check chain state, ownership, rescue availability, traits,
or image rendering.
...
fonts, mooncat parser, Web3 code, worker files, or transaction code. It adapts
the visible scan/start/stop/found-state behavior and recreates the moon/space
scanner styling with CSS and canvas.
...
- Does not infer image rendering, traits, parser behavior, ownership, or current
  chain state.
